function [modelParameters, firingData] = positionEstimatorTraining(trainingData)
    ...
    for trimmer = timePoints
        ...
        % PCA and LDA dimensions
        pcaDim = 62;
        ldaDim = 4;
        [princComp, eVals] = covPCA(firingData, 62);
    ...
    for j = 1:length(timePoints)
        ...
        for i = 1:noDirections
            ...
            % Train linear regression models using ridge regression (L2 regularization)
            beta_x = (W_dir' * W_dir + lambda * eye(size(W_dir,2))) \ (W_dir' * x4pcr);
            beta_y = (W_dir' * W_dir + lambda * eye(size(W_dir,2))) \ (W_dir' * y4pcr);
