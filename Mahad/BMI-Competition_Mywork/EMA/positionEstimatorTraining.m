...
% Determine the minimum spike length across all trials to avoid exceeding array bounds
minTime = inf;
...
% -------------- Preprocess spike data: Binning + Sqrt + Smoothing ---------------
binnedTrials   = binAndTransformSpikes(trainingData, timeBin, true);
smoothedTrials = smoothFiringRatesEMA(binnedTrials, timeBin, gaussianScale);
...
for directionIdx = 1:numDirections
    for trialIdx = 1:numTrainingTrials
        for timeIdx = 1:(endBin/timeBin)
            ...
            aggregatedFiringRates(rowStart:rowEnd, colIndex) = ...
                smoothedTrials(trialIdx, directionIdx).rates(:, timeIdx);
        end
    end
end

% Remove neurons with very low average firing rate
lowFiringNeurons = [];
...
for currentTimeBinLimit = timeBinLimits
    ...
    % Remove low-firing neurons from aggregated data
    removalIndices = [];
    ...
    for directionIdx = 1:numDirections
        classMeanMatrix(:, directionIdx) = mean(aggregatedFiringRates(:, ...
            numTrainingTrials*(directionIdx-1)+1 : numTrainingTrials*directionIdx), 2);
    end
    ...
    optimalProjection    = principalComponents(:,1:numPCADimensions) * ...
                           ldaEigenVectors(:, ldaSortIndices(1:numLDADimensions));
...
[meanPosX, meanPosY, resampledPosX, resampledPosY] = padAndResamplePositions(trainingData, ...
    numDirections, numTrainingTrials, timeBin);

xTestIntervals = resampledPosX(:, [startBin:timeBin:endBin]/timeBin, :);
yTestIntervals = resampledPosY(:, [startBin:timeBin:endBin]/timeBin, :);

% Create a time vector matching the firing data
timeBinsForFiring = repelem(timeBin:timeBin:endBin, numNeurons);
testingTimeBins   = startBin:timeBin:endBin;

% We'll re-aggregate firing rates again for PCR
% (Use the last aggregatedFiringRates from the final iteration if needed)
% but that large matrix is still in the workspace.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Function: binAndTransformSpikes
% Re-bins the spike data and optionally applies a square-root transformation.
function processedTrials = binAndTransformSpikes(rawTrials, binInterval, applySqrt)
    ...
    for colIdx = 1:size(rawTrials,2)
        for rowIdx = 1:size(rawTrials,1)
            spikeMatrix = rawTrials(rowIdx, colIdx).spikes;  % rows: neurons, cols: time
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Function: smoothFiringRatesGaussian
% Computes smoothed firing rates using a Gaussian kernel.
function finalTrials = smoothFiringRatesGaussian(binnedTrials, binInterval, gaussianScale)
    finalTrials          = struct;
        ...
        for rowIdx = 1:size(binnedTrials,1)
            ...
            for neuronIdx = 1:numNeurons
                smoothedRates(neuronIdx,:) = ...
                    conv(binnedTrials(rowIdx, colIdx).spikes(neuronIdx,:), ...
                         gaussianKernel, 'same') / (binInterval/1000);
            end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Function: padAndResamplePositions
% Pads each trial's hand position data to a common length and downsamples it.
function [meanPosX, meanPosY, resampledPosX, resampledPosY] = padAndResamplePositions(positionData, ...
                                numDirections, numTrainingTrials, binInterval)
    ...
    for directionIdx = 1:numDirections
        for trialIdx = 1:numTrainingTrials
            ...
            % Downsample the padded trajectory
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Function: smoothFiringRatesEMA
% Uses an EMA kernel (exponential smoothing) instead of Gaussian.
function finalTrials = smoothFiringRatesEMA(binnedTrials, binInterval, gaussianScale)

    % This is just an example scaling: alpha related to a "windowSize" notion
    windowSize = 26;  % a chosen fixed size
    if windowSize < 1, windowSize = 1; end
    alpha = 30 / (windowSize + 1);
