...
if trialDuration <= lastbinsize
    ...
    if targetAngles(predictedLabel) == 30
        regressionCoeffX = regressionCoeffX * 1.3;  
        regressionCoeffY = regressionCoeffY * 0.7;
    elseif targetAngles(predictedLabel) == 70
        regressionCoeffX = regressionCoeffX * 1.2;  
        regressionCoeffY = regressionCoeffY * 1.55;
    elseif targetAngles(predictedLabel) == 110
        regressionCoeffX = regressionCoeffX * 0.5;  
        regressionCoeffY = regressionCoeffY * 1.1;
    elseif targetAngles(predictedLabel) == 150
        regressionCoeffX = regressionCoeffX * 0.2;  
        regressionCoeffY = regressionCoeffY * 0.4;
    elseif targetAngles(predictedLabel) == 190
        regressionCoeffX = regressionCoeffX * 0.6;  
        regressionCoeffY = regressionCoeffY * 0.35;
    elseif targetAngles(predictedLabel) == 230
        regressionCoeffX = regressionCoeffX * 0.7;  
        regressionCoeffY = regressionCoeffY * 0.6;
    elseif targetAngles(predictedLabel) == 310
        regressionCoeffY = regressionCoeffY * 1.8;
    elseif targetAngles(predictedLabel) == 350
        regressionCoeffX = regressionCoeffX * 1.5;  
        regressionCoeffY = regressionCoeffY * 0.4; 
    end
    ...
    if targetAngles(predictedLabel) == 30
        predictedX = predictedX * 0.98; 
        predictedY = predictedY * 0.98;
    elseif targetAngles(predictedLabel) == 70
        predictedX = predictedX * 1.02;  
        predictedY = predictedY * 0.97;
    elseif targetAngles(predictedLabel) == 110
        predictedX = predictedX * 0.99;  
    elseif targetAngles(predictedLabel) == 230
        predictedX = predictedX * 0.99; 
        predictedY = predictedY * 0.98;
    elseif targetAngles(predictedLabel) == 310
