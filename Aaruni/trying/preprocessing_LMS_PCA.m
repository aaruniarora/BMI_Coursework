...
for trial_idx = 1:size(Y_data_reshaped, 3)
    ...
    plot(Y_data_reshaped(1,:,trial_idx), Y_data_reshaped(2,:,trial_idx), 'b', 'LineWidth', 1, 'DisplayName', 'Actual Position');
    
    % Plot predicted trajectory in red
    plot(Y_pred_reshaped(1,:,trial_idx), Y_pred_reshaped(2,:,trial_idx), 'r', 'LineWidth', 1, 'DisplayName', 'Decoded Position');
end
