function modelParameters = positionEstimatorTraining(training_data)
   ...
   % Store in modelParameters
    ...
    lambda_ridge = 0.1; lambda_lasso = 0.1;
    ...
    for dir_idx = 1:directions
        ...
        for win_idx = 1:((stop_idx-start_idx)/bin_group)+1
            ...
            [ridge_coeff_X, ridge_coeff_Y, lasso_coeff_X, lasso_coeff_Y, win_firing] = calc_reg_coeff(win_idx, time_division, labels, dir_idx, ...
                spikes_matrix, pca_threshold, time_interval, curr_X_pos, curr_Y_pos, lambda_ridge, lambda_lasso);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    assignin('base', "principal_components_inv", principal_components');
    assignin('base', "centered_X", centered_X);
    ...
    reg_mat_ridge = pinv(principal_components * principal_components') * principal_components;
...
function beta = lasso_regression(X, y, lambda)
    ...
    [n, p] = size(X);
    assignin('base', 'Xbeta', X);
    beta = zeros(p, 1);
    max_iter = 1000;
    tol = 1e-6;

    if n == 0
       error('X is empty! No samples to run LASSO on.');
    end
    if any(isnan(X(:))) || any(isinf(X(:)))
       error('X has NaN or Inf');
    end
    if any(isnan(y(:))) || any(isinf(y(:)))
       error('y has NaN or Inf');
    end
