function modelParameters = positionEstimatorTraining(training_data)
    ...
    bin_group = 20;
    global ALPHA
    alpha = ALPHA; % arbitrary value decided through multiple trials
   ...
   % Store in modelParameters
    ...
    [spikes_mat, labels] = extract_features(preprocessed_data, orig_neurons, stop_idx/bin_group, 'nodebug');
    ...
    % spikes_mat(removed_neurons, :) = [];
    % spikes_mat = reshape(spikes_mat, [], 1);
    clear spikes_mat

    % %% K-means clustering on the LDA output (after dimensionality reduction)
    % 
    % pca_threshold = 0.44; % =40 for cov and =0.95 for svd
    % [coeff_k, score_k, nPC_k] = perform_PCA(spikes_mat, pca_threshold, 'cov', 'nodebug');
    % 
    % modelParameters.kmean_pca = coeff_k;
    % 
    % lda_dim = 6;
    % [outputs_k, weights_k] = perform_LDA(spikes_mat, score_k, labels, lda_dim, training_length, 'nodebug');
    % 
    % modelParameters.kmean_lda = outputs_k;
    % modelParameters.kmean_lda_w = weights_k;
    % 
    % numClusters = 8;  % Choose an appropriate number of clusters
    % [clusterLabels, centroids] = kmeansClustering(outputs_k, numClusters);  % Perform clustering on the LDA output
    % 
    % %Store K-means results in modelParameters
    % modelParameters.kmean_l.kMeansLabels = clusterLabels;
    ...
    for curr_bin = 1: length(num_bins)
     ...
     % =40 for cov and =0.95 for svd
        ...
        modelParameters.classify(curr_bin).pca_coeff = coeff;
    ...
    [xPos, yPos, formatted_xPos, formatted_yPos,xStd,yStd] = handPos_processing(training_data, bin_group, num_bins*bin_group);

    modelParameters.stdx = mean(xStd,1);
    modelParameters.stdy = mean(yStd,1);
...
function [clusterLabels, centroids] = kmeansClustering(data, numClusters)
    % K-means clustering function without toolbox
    %
    % Inputs:
    %   data           - Matrix of data points (D x N), where D is the number of features
    %                    and N is the number of data points.
    %   numClusters    - The number of clusters to form.
    %
    % Outputs:
    %   clusterLabels  - Cluster assignments for each data point (N x 1).
    %   centroids      - Centroids of the clusters (D x numClusters).
    data = data';
    % Initialize centroids randomly from the data
    [numFeatures, numDataPoints] = size(data);

    centroids = data(:, randperm(numDataPoints, numClusters));  % Randomly select centroids
    
    % Initialize variables
    clusterLabels = zeros(1, numDataPoints);
    prevCentroids = centroids;
    
    maxIter = 1000;  % Maximum iterations to prevent infinite loops
    tol = 1e-6;      % Tolerance for convergence
    
    for iter = 1:maxIter
        % Step 1: Assign each point to the closest centroid
        distances = zeros(numClusters, numDataPoints);
        for c = 1:numClusters
            distances(c, :) = sum((data - centroids(:, c)).^2, 1);  % Euclidean distance squared
        end
        [~, clusterLabels] = min(distances, [], 1);  % Assign labels to the closest centroid
        
        % Step 2: Update centroids by computing the mean of points in each cluster
        for c = 1:numClusters
            pointsInCluster = data(:, clusterLabels == c);  % Points assigned to cluster c
            if ~isempty(pointsInCluster)
                centroids(:, c) = mean(pointsInCluster, 2);  % Update centroid
            end
        end
        
        % Check for convergence (if centroids do not change significantly)
        centroidShift = sum(sum((centroids - prevCentroids).^2));
        if centroidShift < tol
            break;
        end
        prevCentroids = centroids;
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    elseif strcmp(method, 'kpca')
        ...
        sigma = 1;  % You can tune this hyperparameter
        K = rbf_kernel(data, sigma);
        ...
        % Determine number of PCs based on variance threshold
        explained_variance = cumsum(d) / sum(d);
        nPC = find(explained_variance >= threshold, 1);

        % Compute transformed scores
        score = K * V(:, 1:nPC);
        coeff = V(:, 1:nPC);  % Eigenvectors of the kernel matrix
    end
    ...
    % Compute the squared Euclidean distance matrix without using pdist2
    ...
    for i = 1:N
        for j = 1:N
            dist_sq = sum((X(i,:) - X(j,:)).^2);  % Squared Euclidean distance
            K(i,j) = exp(-dist_sq / (2 * sigma^2));  % RBF kernel formula
        end
    end
end
end
...
%   FilteredFiring : Neural data filtered by time and direction, used for regression.
    ...
    % centeredX = currentXPositions(:,timeWindowIndex) - mean(currentXPositions(:,timeWindowIndex));
    % centeredY = currentYPositions(:,timeWindowIndex) - mean(currentYPositions(:,timeWindowIndex));
    ...
    assignin('base','princ_comp',principalComponents)
