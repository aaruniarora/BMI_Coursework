function [x, y, modelParameters] = positionEstimator(test_data, modelParameters)
   ...
   if curr_bin <= stop_idx 
    ...
    x = calculatePosition(spikes_test, meanFiring, bx, avX, curr_bin,reg_meth,polyDegree);
    y = calculatePosition(spikes_test, meanFiring, by, avY, curr_bin,reg_meth,polyDegree);
end
...
function pos = calculatePosition(neuraldata, meanFiring, b, av, curr_bin,reg_meth,polyd)
    if strcmp(reg_meth, 'poly')
    firingVector = neuraldata(1:length(b));  
    ...
    for d = 1:polyd
        polyFiringVector = polyFiringVector + (firingVector - mean(meanFiring)).^d;
    end

    % Predict position using polynomial regression
    pos = polyFiringVector' * b + av;
    else
