...
function [principalComponents, eigenValues, sortIndices, eigenVectors] = performPCA(dataMatrix)
    % Remove mean across trials
    centeredData = dataMatrix - mean(dataMatrix,2);
    % Compute covariance matrix
    covarianceMatrix = (centeredData' * centeredData) / size(dataMatrix,2);
    % Eigen decomposition of the covariance matrix
    [eigenVectors, eigenValueMatrix] = eig(covarianceMatrix);
    % Sort eigenvalues and eigenvectors in descending order
    [~, sortIndices] = sort(diag(eigenValueMatrix), 'descend');
    ...
    sortedEigenValues = diag(eigenValueMatrix);
    eigenValues = diag(sortedEigenValues(sortIndices));
end
