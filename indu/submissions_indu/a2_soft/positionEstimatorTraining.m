   ...
   % Store in modelParameters
    ...
    for dir_idx = 1:directions%length(reaching_angles)
        ...
        for win_idx = 1:((stop_idx-start_idx)/bin_group)+1
            ...
            [reg_coeff_X, reg_coeff_Y, windowedFiring] = calcRegressionCoefficients(win_idx, time_division, labels, dir_idx, spikes_matrix, pca_threshold, time_interval, curr_X_pos, curr_Y_pos);
            
            % Store the calculated regression coefficients and the mean windowed firing rates in the model parameters structure.
            % x_add = 1;
            % y_add = 1;
            modelParameters.pcr(dir_idx,win_idx).bx = reg_coeff_X;%*x_add;
            modelParameters.pcr(dir_idx,win_idx).by = reg_coeff_Y;%*y_add;
            modelParameters.pcr(dir_idx,win_idx).fMean = mean(windowedFiring,2);
            modelParameters.pcr(dir_idx,win_idx).fstd = std(windowedFiring,0,2);
