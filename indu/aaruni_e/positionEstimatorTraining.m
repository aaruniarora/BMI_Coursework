   ...
   % Store in modelParameters
    ...
    for directionIndex = 1:length(reaching_angles)
        ...
        for timeWindowIndex = 1:((stop_idx-start_idx)/bin_group)+1
            ...
            [regressionCoefficientsX, regressionCoefficientsY, windowedFiring,Bx,By,A,Q,R,H,P,GPR_hyperparams, GPR_predictions_X, GPR_predictions_Y] = calcRegressionCoefficients(timeWindowIndex, time_division, labels, directionIndex, spikes_matrix, pca_threshold, Interval, currentXPositions, currentYPositions);
            ...
            modelParameters.poly(directionIndex, timeWindowIndex).bx = Bx;
            modelParameters.poly(directionIndex, timeWindowIndex).by = By;
            ...
            modelParameters.kalman(directionIndex, timeWindowIndex).A = A;
            modelParameters.kalman(directionIndex, timeWindowIndex).Q = Q;
            modelParameters.kalman(directionIndex, timeWindowIndex).R = R;
            modelParameters.kalman(directionIndex, timeWindowIndex).H = H;
            modelParameters.kalman(directionIndex, timeWindowIndex).P = P;

            modelParameters.gpr(directionIndex,timeWindowIndex).hyper = GPR_hyperparams;
            modelParameters.gpr(directionIndex,timeWindowIndex).x_pred = GPR_predictions_X;
            modelParameters.gpr(directionIndex,timeWindowIndex).y_pred = GPR_predictions_Y;
...
function [regressionCoefficientsX, regressionCoefficientsY, FilteredFiring,Bx,By,A,Q,R,H,P,GPR_hyperparams, GPR_predictions_X, GPR_predictions_Y] = calcRegressionCoefficients(timeWindowIndex, timeDivision, labels, directionIndex, neuraldata, pca_dimension, Interval, currentXPositions, currentYPositions)
...
%   FilteredFiring : Neural data filtered by time and direction, used for regression.
    ...
    % disp('centredwindowfiring')
    % size(centeredWindowFiring)
    ...
    Z = principalVectors(:, 1:pca_dimension)' * (centeredWindowFiring);
    ...
    Bx = (Z_poly * Z_poly') \ (Z_poly * centeredX);
    By = (Z_poly * Z_poly') \ (Z_poly * centeredY);

    % Kalman Filter Parameters (Stored for Future Use)
    ...
    R = eye(size(centeredWindowFiring, 1)) * 0.1; % Measurement noise covariance
    H = randn(size(centeredWindowFiring, 1), 4); % Measurement model (to be trained)
    P = eye(4); % Initial state covariance

    % Gaussian Process Regression (GPR) Implementation
    % Kernel function: Squared Exponential (RBF)
    kernel = @(x1, x2, l, sigma_f) sigma_f^2 * exp(-0.5 * sum((x1 - x2).^2) / l^2);
    
    % Hyperparameters
    l = 1;  % Length-scale
    sigma_f = 3;  % Signal variance
    sigma_n = 0;  % Noise variance

    num_samples = size(centeredWindowFiring, 2);

    K = zeros(num_samples, num_samples);
    for i = 1:num_samples
        for j = 1:num_samples
            K(i, j) = kernel(centeredWindowFiring(i, :), centeredWindowFiring(j, :), l, sigma_f);
        end
    end
    K = K + sigma_n^2 * eye(num_samples);  % Add noise variance to the diagonal
        
    % Compute the inverse covariance matrix
    K_inv = pinv(K);

    assignin('base','centeredX',centeredX)
    
    % Compute predictions for X and Y
    GPR_predictions_X = K_inv * centeredX;
    GPR_predictions_Y = K_inv * centeredY;

    % Normalize the predictions (if needed, depending on your use case)
    GPR_predictions_X = (GPR_predictions_X - mean(GPR_predictions_X)) / std(GPR_predictions_X);
    GPR_predictions_Y = (GPR_predictions_Y - mean(GPR_predictions_Y)) / std(GPR_predictions_Y);

    assignin('base','gpr_x',GPR_predictions_X)
    assignin('base','gpr_y',GPR_predictions_Y)
    
    % Store GPR hyperparameters
    GPR_hyperparams.l = l;
    GPR_hyperparams.sigma_f = sigma_f;
    GPR_hyperparams.sigma_n = sigma_n;
